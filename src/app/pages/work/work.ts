  ...
  if(overlay.getProjection()) {
    ...
    markers.forEach(function(marker){
      ...
      var infowindow = new google.maps.InfoWindow({
        content: "<project-card></project-card>",
        maxWidth: 200,
        pixelOffset: new google.maps.Size(mapSize.width/2, mapSize.height/2)
      });
...
function initMap(){
  ...
  //       content: "<project-card></project-card>",
  //       maxWidth: 200,
  //       pixelOffset: new google.maps.Size(mapSize.width/2-p.x, mapSize.height*0.75-p.y)
  //     });
  //     marker.addListener('click', function(){
  //       infowindow.open(map, marker);
  //     });
  //   });
  // });
  // google.maps.event.addListener(map, 'idle', setMarkers());
